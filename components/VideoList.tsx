  ...
  return (
    <div className="flex flex-col h-full relative">
      ...
      <div className="px-4 space-y-4 overflow-y-auto pb-32">
        {VIDEOS.map((video) => (
          <div key={video.id} className="liquid-glass rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all group">
            <div className="relative">
              <img src={video.thumbnail} alt={video.title} className="w-full h-48 object-cover opacity-100 transition-opacity" />
              <div className="absolute inset-0 bg-black/10 flex items-center justify-center group-hover:bg-black/20 transition-colors">
                <div className="w-12 h-12 bg-white/30 backdrop-blur-md rounded-full flex items-center justify-center border border-white/60 shadow-lg">
                   <Play className="w-5 h-5 text-white fill-white ml-1" />
                </div>
              </div>
              <span className="absolute bottom-2 right-2 bg-black/60 backdrop-blur-sm text-white text-[10px] px-2 py-1 rounded">
                {video.duration}
              </span>
            </div>
            <div className="p-4">
              <h3 className="font-semibold text-slate-800 line-clamp-2 leading-tight">{video.title}</h3>
